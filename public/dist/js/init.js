 ...
 ** ***************************************/
	...
	socket.emit("user", { token: document.cookie });
	...
	socket.on("get_message", (message) => {
		console.log(message)
		if(message.send) {
			chat[message.send] += `<li class="friend">
			<div class="friend-msg-wrap">
			<img class="user-img img-circle block pull-left"  src="${avatar}" alt="user"/>
			<div class="msg pull-left"> 
				<p>${message.message}</p>
				<div class="msg-per-detail  text-right">
					<span class="msg-time txt-grey">${(new Date()).toTimeString().split(' ')[0]}</span>
				</div>
			</div>
			<div class="clearfix"></div>
				</div>	
			</li>`;
			if(message.send == id_user) {
			$(".chatapp-chat-nicescroll-bar").append(`<li class="friend">
			<div class="friend-msg-wrap">
			<img class="user-img img-circle block pull-left"  src="${avatar}" alt="user"/>
			<div class="msg pull-left"> 
				<p>${message.message}</p>
			...
			$("#tb"+message.send).html(message.message);
		}
		else {
		$(".chatapp-chat-nicescroll-bar").append(`<li class="friend">
			<div class="friend-msg-wrap">
			<img class="user-img img-circle block pull-left"  src="/public/upload/admin.png" alt="user"/>
			<div class="msg pull-left"> 
				<p>${message.message}</p>
				<div class="msg-per-detail  text-right">
					<span class="msg-time txt-grey">${(new Date()).toTimeString().split(' ')[0]}</span>
		...
		//$('<li class="self mb-10"><div class="self-msg-wrap"><div class="msg block pull-right">' + message.message + '<div class="msg-per-detail mt-5"><span class="msg-time txt-grey">3:30 pm</span></div></div></div><div class="clearfix"></div></li>').insertAfter(".fixed-sidebar-right .chat-content  ul li:last-child");
	})
	...
	$(document).on("keypress", "#input_msg_send_chatapp", function (e) {
		if ((e.which == 13) && (!$(this).val().length == 0)) {
			let obj = { message: $(this).val(), token: document.cookie };
			if (id_user) {
				obj.sendTo = id_user;
			}
			socket.emit("message", obj);
			$('<li class="self mb-10"><div class="self-msg-wrap"><div class="msg block pull-right">' + $(this).val() + '<div class="msg-per-detail mt-5"><span class="msg-time txt-grey">'+(new Date()).toTimeString().split(' ')[0]+'</span></div></div></div><div class="clearfix"></div></li>').insertAfter(".chat-for-widgets-1 .chat-content  ul li:last-child");
			if(id_user) {
				chat[id_user] = $(".chatapp-chat-nicescroll-bar");
			}
			$(this).val('');
		} else if (e.which == 13) {
